...
def process_data(data: list, dbh, id_collection: str, filepath: str) -> tuple:
    ...
    for idx, document in enumerate(data):
        biomarker_id, hash_value, collision, core_values_str = idh.id_assign(document, dbh, id_collection)
        document['biomarker_id'] = biomarker_id
        # if there is a hash collision, handle appropriately 
        if collision:
            existing_entry = idh.get_record_by_id(biomarker_id, dbh, id_collection)
            if existing_entry:
