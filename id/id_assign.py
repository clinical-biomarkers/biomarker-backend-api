...
def main():
    ...
    misc_fns.setup_index(
        dbh, "biomarker_canonical_id", second_level_id_collection, "canonical_1"
    )
    ...
    for fp in files:
        ...
        if not isinstance(data, list):
            logging.error(
                f"Error reading data from file `{fp}`, expected `list`, got `{type(data)}`."
            )
            print(
                f"Error reading data from file `{fp}`, expected `list`, got `{type(data)}`. Skipping..."
            )
            continue
    ...
    if id_backend.dump_id_collection(
        connection_string, canonical_id_collection_local_path, canonical_id_collection
    ):
