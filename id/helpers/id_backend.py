...
def process_file_data(data: list,
    ...
    for idx, document in enumerate(data):
        ...
        canonical_id, second_level_id, second_level_collision, hash_value, core_str = _id_assign(document, dbh, can_id_coll, second_id_coll) 
        ...
        if second_level_collision:
            ...
            existing_unreviewed_records = get_record_by_id(second_level_id, False, dbh, unreviewed_coll)
            base_collision_obj = {
    ...
    logging.info(f'Finished assigning ID\'s for {filepath}.\n\t\
        Soft collisions: {standard_collision_count}\n\t\
        Hard collisions: {hard_collision_count}'
    )
    return updated_data

def _id_assign(document: dict, dbh: Database, canonical_id_coll: str = CANONICAL_DEFAULT, second_id_coll: str = SECOND_DEFAULT) -> tuple:
    ...
    '''
    canonical_id, hash_value, core_values_str, canonical_collision = canonical.get_ordinal_id(document, dbh, canonical_id_coll)
